...
function salvarDoador(string $nome, $email, string $telefone, string $tipo, float $valor, string $mpId): void
{
    try {
        $pdo  = getDbConnection();
        $stmt = $pdo->prepare("
            INSERT INTO doadores (nome, email, telefone, tipo, valor, status, mp_id)
            VALUES (:nome, :email, :telefone, :tipo, :valor, 'pendente', :mp_id)
        ");
        $stmt->execute([
            ':nome'     => $nome,
            ':email'    => $email ?: '',
...
if ($tipo === 'unica') {
    $payload = [
        'items' => [[
            'title'       => 'Doação Fórum Animal Bioética',
            'quantity'    => 1,
            'unit_price'  => $valor,
            'currency_id' => 'BRL',
        ]],
    ...
    salvarDoador($nome, $email, $telefone, 'unica', $valor, $response['id'] ?? '');
...
if ($tipo === 'mensal') {
    ...
    $payload = [
        'reason'           => 'Doação Mensal — Fórum Animal Bioética',
        'payer_email'      => $email,
    ...
    salvarDoador($nome, $email, $telefone, 'mensal', $valor, $response['id'] ?? '');
